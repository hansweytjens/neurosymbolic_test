...
def create_ngrams(data, train_ids, val_ids, test_ids, window_size=30):
...
def preprocess_eventlog(data, seed=42, train_ids=None, val_ids=None, test_ids=None):
    ...
    # Numerical: budget and total declared
    if "case:RequestedBudget" in data.columns:
        scaler_budget = MinMaxScaler()
        data["case:RequestedBudget"] = data["case:RequestedBudget"].ffill().fillna(0)
        data["case:RequestedBudget"] = scaler_budget.fit_transform(data[["case:RequestedBudget"]])
        scalers["case:RequestedBudget"] = scaler_budget

    if "case:TotalDeclared" in data.columns:
        scaler_total = MinMaxScaler()
        data["case:TotalDeclared"] = data["case:TotalDeclared"].ffill().fillna(0)
        data["case:TotalDeclared"] = scaler_total.fit_transform(data[["case:TotalDeclared"]])
        scalers["case:TotalDeclared"] = scaler_total
    ...
    for col in ["case:RequestedBudget", "case:TotalDeclared"]:
